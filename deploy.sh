...
oc new-app --template=amq63-basic -p MQ_PROTOCOL=openwire,amqp,stomp,mqtt,hornetq -p MQ_QUEUES=demoQueue -p MQ_USERNAME=user -p MQ_PASSWORD=pass123
...
oc new-app --template=mysql-ephemeral -p MYSQL_DATABASE=demodb -p MYSQL_USER=dbuser -p MYSQL_PASSWORD=dbpass123 -p MYSQL_ROOT_PASSWORD=rootpass123
